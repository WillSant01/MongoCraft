...
def ricerca_per_mese(mese_anno):
    try:
        # Converti la stringa del mese in un oggetto datetime
        data = datetime.strptime(mese_anno, '%Y-%m')
        
        # Ottieni il primo e l'ultimo giorno del mese
        primo_giorno = data.replace(day=1)
        ultimo_giorno = (data.replace(month=data.month % 12 + 1, day=1) - timedelta(days=1)) if data.month != 12 else data.replace(month=12, day=31)
        ...
        print(f"Errore durante la ricerca per mese: {e}")
        return []
...
def main():
    while True:
        ...
        print("5| Vedi concerti per mese")
        print("6| Esci")
        ...
        match scelta:
            ...
            case '5':
                print("\n<<< Inserisci il mese e l'anno (YYYY-MM): ")
                mese_anno = input("\n> ")
                
                risultati_mese = ricerca_per_mese(mese_anno)
                
                if risultati_mese:
                    for concerto in risultati_mese:
                        mostra_concerto(concerto)
                else:
                    print("\n<<< Non è stato trovato nessun concerto che corrisponda ai criteri di ricerca")
